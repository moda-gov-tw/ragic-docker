...
public final class DeepLinkRedirect_jsp extends org.apache.jasper.runtime.HttpJspBase
...
if(url.startsWith("https")) {
    deepLinkUrl = url.replaceFirst("https", "ragic");
} else if(url.startsWith("http")) {
    deepLinkUrl = url.replaceFirst("http", "ragic");
} else {
    return;
}
String qs=request.getParameter("qs");
if(qs==null) return;
JSONArray qsArr = new JSONArray(qs);
boolean first = true;
for(int i = 0; i < qsArr.length(); i++) {
    String q = qsArr.optString(i, null);
    if(q == null) continue;
    deepLinkUrl += first? "?": "&";
    deepLinkUrl += q;
    first = false;
...
if(!install) {
      out.write("\r\nwindow.location=\"");
      out.print(deepLinkUrl);
      out.write("\";\r\nwindow.setTimeout(function() {\r\n    try {\r\n        mobileAppRedirectInstall('");
