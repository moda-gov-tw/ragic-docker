/*
* from //cdnjs.cloudflare.com/ajax/libs/authy-forms.js/2.2/form.authy.min.js
* Used to create dropdown menu of country codes
* */
var RagicCountryCodeDataList = (function () {
    var dataList = {};
    dataList.UI = function () {
        var absolutePosFor, buildItem, countriesList, disableAutocompleteAuthyToken, findAndSetupCountries, getKeyCode,
            hideAutocompleteList, processKey13, processKey38, processKey40, t, setActive, setCountryField,
            setupAuthyTokenValidation, setupCellphoneValidation, setupCountriesDropdown, setupCountriesDropdownPosition,
            setupEvents, initDefaultCountryCode;
        t = this;

        countriesList = [{country: "United States of America (+1)", code: "1"}, {
            country: "Canada (+1)",
            code: "1"
        }, {country: "Russia (+7)", code: "7"}, {country: "Kazakhstan (+7)", code: "7"}, {
            country: "Egypt (+20)",
            code: "20"
        }, {country: "South Africa (+27)", code: "27"}, {
            country: "Greece (+30)",
            code: "30"
        }, {country: "Netherlands (+31)", code: "31"}, {country: "Belgium (+32)", code: "32"}, {
            country: "France (+33)",
            code: "33"
        }, {country: "Spain (+34)", code: "34"}, {country: "Hungary (+36)", code: "36"}, {
            country: "Italy (+39)",
            code: "39"
            ...
            code: "44"
        }, {country: "Jersey (+44)", code: "44"}, {country: "Denmark (+45)", code: "45"}, {
            country: "Sweden (+46)",
            code: "46"
        }, {country: "Norway (+47)", code: "47"}, {country: "Poland (+48)", code: "48"}, {
            country: "Germany (+49)",
            code: "49"
        }, {country: "Peru (+51)", code: "51"}, {country: "Mexico (+52)", code: "52"}, {
            country: "Cuba (+53)",
            code: "53"
        }, {country: "Argentina (+54)", code: "54"}, {country: "Brazil (+55)", code: "55"}, {
            country: "Chile (+56)",
            code: "56"
            ...
            country: "Vietnam (+84)",
            code: "84"
        }, {country: "China (+86)", code: "86"}, {country: "Turkey (+90)", code: "90"}, {
            country: "India (+91)",
            code: "91"
        }, {country: "Pakistan (+92)", code: "92"}, {
            country: "Afghanistan (+93)",
            code: "93"
        }, {country: "Sri Lanka (+94)", code: "94"}, {country: "Myanmar (+95)", code: "95"}, {
            country: "Iran (+98)",
            code: "98"
        }, {country: "Morocco (+212)", code: "212"}, {
            country: "Algeria (+213)",
            code: "213"
        }, {country: "Tunisia (+216)", code: "216"}, {country: "Libya (+218)", code: "218"}, {
            country: "Gambia (+220)",
            code: "220"
            ...
            code: "226"
        }, {country: "Niger (+227)", code: "227"}, {country: "Togo (+228)", code: "228"}, {
            country: "Benin (+229)",
            code: "229"
            ...
            code: "253"
        }, {country: "Kenya (+254)", code: "254"}, {country: "Tanzania (+255)", code: "255"}, {
            country: "Uganda (+256)",
            code: "256"
            ...
            country: "Iceland (+354)",
            code: "354"
        }, {country: "Albania (+355)", code: "355"}, {country: "Malta (+356)", code: "356"}, {
            country: "Cyprus (+357)",
            code: "357"
            ...
            code: "960"
        }, {country: "Lebanon (+961)", code: "961"}, {country: "Jordan (+962)", code: "962"}, {
            country: "Syria (+963)",
            code: "963"
            ...
            country: "Israel (+972)",
            code: "972"
        }, {country: "Bahrain (+973)", code: "973"}, {country: "Qatar (+974)", code: "974"}, {
            country: "Bhutan (+975)",
            code: "975"
        ...
        processKey40 = function (listId) {
            ...
            for (_i = 0, _len = countriesArr.length; _i < _len; _i++) {
                ...
                if (li.className === "active" && countriesArr.length > (i + 1)) {
                    ...
                    t.autocomplete(activeElement, false);
                    break;
                }
                i++;
            }
        ...
        setupEvents = function (countriesInput, listId) {
            ...
            //     if(countriesInput.value === "") return;
        ...
        setupCountriesDropdown = function (countriesSelect, listId) {
            var buf, classActive, countries, countriesAutocompleteList, countriesDropdown, countriesInput,
                countriesInputType, countryCodeValue, i, name, placeholder;
            if (!countriesSelect) {
                return;
            }
            countries = [];
        ...
        setupCountriesDropdownPosition = function (countriesInput, countriesDropdown) {
            ...
            return countriesDropdown.setAttribute("style", "width: " + (width - 5) + "px; top: " + (pos[0] + 2 + countriesInput.offsetHeight) + "px; left: " + (pos[1]) + "px;")
        };
            ...
            if (countryCode !== "") {
                ...
                for (_i = 0, _len = countriesList.length; _i < _len; _i++) {
                    ...
                    if (country.code === countryCode) {
                        t.autocomplete(buildItem("active", country, defaultListId), true);
                        break
                    } else {
        ...
        initDefaultCountryCode = function (countryCode) {
            let defaultli = document.querySelector("li[rel='"+ countryCode +"']");
            t.autocomplete(defaultli,true);
        }
        ...
        this.init = function (countryCode) {
            ...
            setCountryField();
            initDefaultCountryCode(countryCode);
            return setupCellphoneValidation();
        };
        this.searchItem = function (listId) {
            var classActive, countriesAutocompleteList, countriesInput, countryItem, countryWords, cw, dropdownMenu,
                firstCountryCodeFound, i, matches, reg, str;
            classActive = "active";
            ...
            if (matches) {
                ...
                return t.setCountryCode(listId, firstCountryCodeFound);
            }
        };
        this.autocomplete = function (obj, hideList) {
            ...
            t.setCountryCode(listId, countryCode);
            if (hideList) {
                hideAutocompleteList(listId);
                countriesInput.blur();
            }
    ...
    dataList.render = function (defaultCC) {
        if (!this.ui) {
            this.ui = new dataList.UI();
            this.ui.init(defaultCC);
        }
        return this.ui;
    };
    return dataList;
...
window.onload = RagicCountryCodeDataList.render;
